...
install.packages(c('devtools', 'AdaptGauss', "pheatmap", 'mixtools', 
                   'qgraph','multtest','Rtools'))
...
InputMatrix <- BRIC::ReadFrom10X_folder(system.file("inst/extdata/10X_3K/folder_10X/"))
```

2. Read from .h5 file.

```{r h5_files, eval= FALSE, echo=TRUE}
InputMatrix <- BRIC::ReadFrom10X_h5(system.file("extdata","5k_pbmc_protein_v3_filtered_feature_bc_matrix.h5", package = "BRIC"))
```
...
```{r create_object, eval= FALSE, echo=TRUE}
object <- CreateBRICObject(InputMatrix)
```
